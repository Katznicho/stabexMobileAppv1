...
const StationsFlatList = ({ stations, position, screen = "HomeStationDetails", searchText, resetSearch, setSearchText }: any) => {
    ...
    const sortStations = async (stationsToSort: any[], userPosition: any) => {
        const sorted = await Promise.all(
            stationsToSort.map(async (station) => {
                const distance = (station.lat && station.lon)
                    ...
                    : Number.POSITIVE_INFINITY; // Set distance to infinity if lat or lon is null
                return { ...station, distance };
            })
        );

        sorted.sort((a, b) => {
            if (!a.lat || !a.lon) return 1; // Move stations with null lat or lon to the end
            if (!b.lat || !b.lon) return -1; // Move stations with null lat or lon to the end
    ...
    return (
        <FlatList
            ...
            data={sortedStations}
            renderItem={({ item }) => <StationListCard
                data={item}
                position={position}
                screen={screen}
            />}
